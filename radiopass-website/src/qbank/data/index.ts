...
/* The authored files. Statements are stripped from the BUILD by
   scripts/vite-strip-paid.mjs; the source stays whole so a fresh clone
   typechecks with nothing generated first. */
...
type RawQuestion = {
  ...
  topic: string
  source: string
  /* Absent on a paid question: the statements are withheld from the bundle and
     come back from premium_content once entitlement is checked. */
  stems?: { label: string; text: string; answer: boolean | null; explanation: string }[]
  keyPoint?: string
  premium?: boolean
  /** How many statements a withheld question has. A count, never the content. */
  stemCount?: number
}
...
function normalise(raw: RawQuestion, index: number): QbQuestion | null {
  ...
  /* A question with no statements is normally malformed and dropped. A PAID
     one has none on purpose — they are withheld from the bundle — and must
     survive as a shell, or the bank silently loses 424 questions and the
     locked cards a visitor should see never render at all. Hydration fills
     the statements in for somebody entitled to them. */
  if (stems.length === 0 && !raw.premium) return null
  ...
  return {
    ...
    stemCount: raw.stemCount ?? stems.length,
...
export function assembleFrom(
  baseSource: unknown,
  extractedSource: unknown,
): QbQuestion[] {
  const baseRaw = baseSource
  const extractedRaw = extractedSource
...
function assemble(): QbQuestion[] {
  return assembleFrom(baseRaw, extractedRaw)
}
...
export const QB_TOTALS = {
  ...
  /* stemCount, not stems.length: a paid question ships without its statements
     and would otherwise count as zero, making the bank look empty on the very
     page asking somebody to buy it. */
  stems: QB_QUESTIONS.reduce((n, q) => n + (q.stemCount ?? q.stems.length), 0),
}
