...
function hasOwnChrome(pathname: string): boolean {
  const exact = ['/', '/physics', '/admin', '/ultrasound-lab', '/mri', '/anatomy']
  const trees = [
    '/question-bank',
    '/ct-lab',
    '/nm-lab',
      ...
      <nav className={open ? 'nav-links open' : 'nav-links'} aria-label="Primary navigation">
        ...
        {user ? (
          ...
          <>
            ...
            <Link to="/pricing" className="button button-small mobile-cta">Start free trial <Icon name="arrow" size={16}/></Link>
          </>
        )}
      </nav>
      <div className="nav-actions">
        {user ? (
          ...
          <>
            ...
            <Link to="/pricing" className="button button-small">Start free trial <Icon name="arrow" size={16}/></Link>
          </>
        )}
...
function VisualLabPage() {
  ...
  const labs: [string, string, string, string, 'xray' | 'mri' | 'ct' | 'us' | 'nm', [string, string][]][] = [
    ...
    ['Ultrasound — the focused course','Every ultrasound concept, one screen at a time: only the idea, the diagram and Next.','US','/ultrasound-lab/focus','us',[['Full laboratory','/ultrasound-lab']]],
    ['Nuclear medicine — the focused lesson','Generator to PET ring: fourteen concepts, one at a time, each one drawn.','NM','/nm-lab','nm',[['▶ Watch the film','/nm-lab/film']]],
  ]
