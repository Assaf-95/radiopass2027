...
/* The authored files. Paid answers are removed from the BUILD by
   scripts/vite-strip-paid.mjs, not from the source — so this repository
   typechecks from a fresh clone with nothing generated first, and only the
   shipped bundle is redacted. assert-no-premium-in-bundle.mjs checks the
   artefact, so if that plugin ever stops matching, the build fails. */
